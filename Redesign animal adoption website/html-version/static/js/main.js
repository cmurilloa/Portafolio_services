  ...
  grid.innerHTML = list.map(pet => `
    <div class="pet-card">
      <div class="pet-image">
        ...
        <span class="pet-age-badge">${pet.age}</span>
      ...
      <div class="pet-info">
        <h3>${pet.name}, ${pet.gender} · ${pet.size}</h3>
        <p class="pet-tagline">${pet.tagline}</p>
        <div class="pet-traits">
          ${pet.traits.map(t => `<span class="pet-trait">${t}</span>`).join('')}
        </div>
...
function filterPets() {
  ...
  const type = getChipValue('type');
  const age = getChipValue('age');
  const gender = getChipValue('gender');
  const size = getChipValue('size');
  const showFavorites = document.getElementById('favoritesFilter')?.classList.contains('active');

  const filtered = allPets.filter(p => {
    const matchSearch = !search || p.name.toLowerCase().includes(search) || p.tagline.toLowerCase().includes(search);
    const matchType = type === 'todos' || p.type === type;
    const matchAge = age === 'todos' || p.age === age;
    ...
    return matchSearch && matchType && matchAge && matchGender && matchSize && matchFavorites;
  });
...
function openModal(id) {
  ...
  info.innerHTML = `
    <div class="pet-modal-badges">
      <span class="pet-badge" style="position:static;display:inline-block">🐾 ${pet.type}</span>
      <span class="pet-age-badge" style="position:static;display:inline-block">${pet.age}</span>
    </div>
    <div>
      <h2 class="pet-modal-title">${pet.name}</h2>
      <p class="pet-modal-subtitle">${pet.gender} · ${pet.size} · ${pet.edad_label}</p>
    </div>
    <p class="pet-modal-tagline">${pet.tagline}</p>
    <div class="pet-traits">
      ${pet.traits.map(t => `<span class="pet-trait">${t}</span>`).join('')}
    </div>
    <p class="pet-modal-desc">${pet.descripcion}</p>
    <div class="pet-modal-adopt">
      <button class="btn-adopt" onclick="window.location.href='formulario-de-adopcion.html?pet=${pet.id}'">
        Iniciar adopción
      </button>
    </div>
  `;
...
function initFormulario() {
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function (e) {
      ...
      const overlay = document.getElementById('adopcion-modal-overlay');
      if (overlay) {
